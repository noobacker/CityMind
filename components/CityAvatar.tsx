...
function CityModel({ pulse, theme, onFocusNeighborhoodsChange }: CityAvatarProps) {
  ...
  const center = useMemo(() => {
    const list = Object.values(pulse.neighborhoods);
    if (list.length === 0) return { lat: 0, lon: 0 };
    const lat = list.reduce((s, n) => s + n.lat, 0) / list.length;
    const lon = list.reduce((s, n) => s + n.lon, 0) / list.length;
    return { lat, lon };
  }, [pulse]);

  // Auto-fit scale based on city extent so any city looks good
  const scale = useMemo(() => {
    const list = Object.values(pulse.neighborhoods);
    if (list.length === 0) return 20;
    const maxLatDelta = Math.max(...list.map((n) => Math.abs(n.lat - center.lat)), 0.01);
    const maxLonDelta = Math.max(...list.map((n) => Math.abs(n.lon - center.lon)), 0.01);
    const maxDelta = Math.max(maxLatDelta, maxLonDelta);
    return Math.min(40, 2.6 / maxDelta);
  }, [pulse, center]);
  ...
  return (
    <group ref={group}>
      ...
      {allNeighborhoods.map(([name, neighborhood]) => {
        const x = (neighborhood.lon - center.lon) * scale;
        const z = -(neighborhood.lat - center.lat) * scale;
        const height = 0.5 + (neighborhood.stress / 40);

        const s = neighborhood.stress;
        let color = '#00ff9d';
        if (s >= 80) color = '#ff3b3b';
        else if (s >= 60) color = '#ff6600';
        else if (s >= 35) color = '#ffcc00';
        ...
        return (
          <group key={name} position={[x, 0, z]}>
            ...
            {(isHighStress || hoveredNeighborhood === name) && (
              <Billboard follow lockX={false} lockY={false} lockZ={false} position={[0, height - 0.1, 0]}>
                <Text
                  position={[0, 0, 0]}
                  fontSize={0.2}
...
export function CityAvatar({ pulse, theme, onFocusNeighborhoodsChange }: CityAvatarProps) {
  return (
    <section
      className="cityAvatarShell"
      style={{ background: 'var(--panel)', border: '1px solid var(--panel-border)', borderRadius: '10px', overflow: 'hidden' }}
      aria-label="Interactive city avatar"
    >
      <div className="cityAvatarStage" style={{ height: '300px', width: '100%', position: 'relative' }}>
        ...
        <div className="cityOrbLabel" style={{ position: 'absolute', bottom: 12, width: '100%', textAlign: 'center', pointerEvents: 'none' }}>
          {pulse.cityName} 3D Topology Map · {pulse.mood} right now · click to focus
        </div>
      </div>
    </section>
  );
